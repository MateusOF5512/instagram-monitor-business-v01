...
def get_data( path_choquei ):
    df = pd.read_csv( path_choquei )
    return df
...
ter_MN=df_map["inter"].iloc[4]; ter_TD=df_map["inter"].iloc[6]; ter_NT=df_map["inter"].iloc[5]; ter_MD=0;
qua_MN=df_map["inter"].iloc[8]; qua_TD=df_map["inter"].iloc[10]; qua_NT=df_map["inter"].iloc[9]; qua_MD=df_map["inter"].iloc[7];
qui_MN=df_map["inter"].iloc[11]; qui_TD=df_map["inter"].iloc[13]; qui_NT=df_map["inter"].iloc[12]; qui_MD=0;
sex_MN=df_map["inter"].iloc[14]; sex_TD=df_map["inter"].iloc[16]; sex_NT=df_map["inter"].iloc[15]; sex_MD=0;
sab_MN=df_map["inter"].iloc[18]; sab_TD=df_map["inter"].iloc[20]; sab_NT=df_map["inter"].iloc[19]; sab_MD=df_map["inter"].iloc[17];
dom_MN=df_map["inter"].iloc[21]; dom_TD=df_map["inter"].iloc[23]; dom_NT=df_map["inter"].iloc[22]; dom_MD=0;
...
ter_MN=df_map["conta"].iloc[4]; ter_TD=df_map["conta"].iloc[6]; ter_NT=df_map["conta"].iloc[5]; ter_MD=0;
qua_MN=df_map["conta"].iloc[8]; qua_TD=df_map["conta"].iloc[10]; qua_NT=df_map["conta"].iloc[9]; qua_MD=df_map["conta"].iloc[7];
qui_MN=df_map["conta"].iloc[11]; qui_TD=df_map["conta"].iloc[13]; qui_NT=df_map["conta"].iloc[12]; qui_MD=0;
sex_MN=df_map["conta"].iloc[14]; sex_TD=df_map["conta"].iloc[16]; sex_NT=df_map["conta"].iloc[15]; sex_MD=0;
sab_MN=df_map["conta"].iloc[18]; sab_TD=df_map["conta"].iloc[20]; sab_NT=df_map["conta"].iloc[19]; sab_MD=df_map["conta"].iloc[17];
dom_MN=df_map["conta"].iloc[21]; dom_TD=df_map["conta"].iloc[23]; dom_NT=df_map["conta"].iloc[22]; dom_MD=0;

matriz2 = [[dom_MD, seg_MD, ter_MD, qua_MD, qui_MD, sex_MD, sab_MD],
          [dom_NT, seg_NT, ter_NT, qua_NT, qui_NT, sex_NT, sab_NT],
          [dom_TD, seg_TD, ter_TD, qua_TD, qui_TD, sex_TD, sab_TD],
          [dom_MN, seg_MN, ter_MN, qua_MN, qui_MN, sex_MN, sab_MN]]

figC1 = go.Figure(data=go.Heatmap(
                   z=matriz2, name="", text=matriz2,
                   x=['Domingo', 'Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado'],
                   y=['Madrugada', 'Noite', 'Tarde', 'Manhã'],
                   texttemplate="%{text}",
                   hovertemplate="</br><b>Dia:</b> %{x}"+
